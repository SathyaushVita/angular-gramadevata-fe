...
export class VillageComponent {
  ...
  ngOnInit(): void {
    ...
    //     return this.templeserviceservice.Village(locationId, term, 1, 50);
    //   })
...
this.searchSubject.pipe(
  ...
  switchMap((term: string) => {
    ...
    if (!term || term.trim() === '' || !locationId) {
      return of(null); // ⛔ stop search API
    }

    this.isSearching = true;

    return this.templeserviceservice
      .Village(locationId, term, 1, 50)
      .pipe(
        finalize(() => {
...
isSearching = false;
  ...
  fallbackImages: string[] = [
    ...
    'assets/default.70924d0b76e43b5bec89.png',
    'assets/villagechatimage.jpg',
    'assets/download21.jpeg',
    'assets/images.jpeg',
  ...
  handleImageErrors(event: Event, index: number) {
    const imgElement = event.target as HTMLImageElement;
    imgElement.src = this.getFallbackImage(index);
  }
...
onSearchChange(term: string) {
  ...
  loadFilteredTemples() {
    ...
    if (this.selectedCategoryId && this.selectedLocationId) {
      ...
      this.templeserviceservice.Village(this.selectedLocationId, this.searchTerm, this.currentPage).subscribe(
        (response) => {
    ...
    this.validatorForm.get('village')?.valueChanges.subscribe(villageID => {
      ...
      if (villageID) {
        ...
        if (this.validatorForm.get('mandal')?.value) {
          ...
          this.applyFilters();
          // Do NOT reset village options here if we assume the user might want to re-select
        } else {
          this.resetVillage();
        }
      }
