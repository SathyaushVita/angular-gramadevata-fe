...
@Component({
  ...
  templateUrl: './districtsview.component.html',
  styleUrl: './districtsview.component.css'
})
export class DistrictsviewComponent {
...
ngOnInit(): void {
  ...
  this.route.paramMap.subscribe(params => {
    ...
    if (this.selectedLocationId) {
      this.validatorForm.patchValue({ district: this.selectedLocationId });
      this.fetchMandals(this.selectedLocationId);
    }
  });

  // 🔁 When district changes, reset mandal/village and close the dropdown
  this.validatorForm.get('district')?.valueChanges.subscribe(districtId => {
    ...
    this.dropdownOpen = false;  // 🔐 this closes the Find box
  });

  // When mandal changes, reset village
  this.validatorForm.get('mandal')?.valueChanges.subscribe(() => {
    this.validatorForm.get('village')?.reset();
  });
 ...
 @ViewChild('dropdownContainer') dropdownContainer!: ElementRef;
  ...
  handleOutsideClick(event: MouseEvent): void {
    ...
    fetchMandals(districtID: string): void {
      this.locationservice.getblocks(districtID).subscribe(
        (res) => {
          if (Array.isArray(res)) {
            ...
            console.error("Expected array for mandals", res);
          }
        },
...
loadFilteredTemples() {
  this.spinner.show();
 if (this.selectedLocationId) {
        this.templeservice.districtfiltertemples(this.selectedLocationId).subscribe(
            (data) => {
...
//     ? `${window.location.origin}/district-details/${temple._id}`
//     : `${window.location.origin}/district-details/`; 
  ...
  if (this.PolicestationForm.valid) {
      this.templeservice.addpolicestation(this.PolicestationForm.value).subscribe(
        ...
        if (this.dialogRef) {
          ...
          error => {
              console.error('Error adding temple:', error);this.spinner.hide(); 
          }
      );
  } else {
      this.PolicestationForm.markAllAsTouched();this.spinner.hide(); 
  }
}
...
//     this.validatorForm.reset(); // Reset the whole form when dropdown opens
//   }
// }
...
private HotelresetFormFields(fields: string[]) {
  ...
  this.tourguideForm.get('country')?.valueChanges.subscribe(countryId => {
      this.tourguideresetFormFields(['state', 'district', 'mandal', 'village_id']);
      if (countryId) {
        this.templeservice.getbyStates(countryId).subscribe(
          (res) => {
            if (Array.isArray(res)) {
  ...
  // State -> Districts
   this.tourguideForm.get('state')?.valueChanges.subscribe(stateId => {
    console.log('Selected State ID:', stateId);
      this.tourguideresetFormFields(['district', 'mandal', 'village_id']);
  ...
  // District -> Mandals
   this.tourguideForm.get('district')?.valueChanges.subscribe(districtId => {
      this.tourguideresetFormFields(['mandal', 'village_id']);
      if (districtId) {
        this.templeservice.getblocks(districtId).subscribe(
          (res) => {
            this.templeMandalOptions = res.map((mandal: any) => ({
  ...
  this.tourguideForm.get('mandal')?.valueChanges.subscribe(mandalId => {
    ...
    private tourguideresetFormFields(fields: string[]) {
    fields.forEach(field => {
