...
@Component({
  ...
  templateUrl: './add-blodd-bank.component.html',
  styleUrl: './add-blodd-bank.component.css'
})
export class AddBloddBankComponent {
...
existingVillageId: string | null = null;
  ...
  constructor(private templeservice:TempleserviceService, private route:ActivatedRoute,
    ...
    private renderer: Renderer2,private spinner: NgxSpinnerService, public dialogRef: MatDialogRef<AddBloddBankComponent>,
...
loadbloodbankEdit(): void {
  this.spinner.show();

  this.templeservice.getbloodbankbyid(this.tourismId).subscribe(
    (res) => {
...
this.spinner.show();
  if (this.isEditMode && this.tourismId) {
    ...
    this.templeservice.updatebloodbank(this.tourismId, payload)
      .subscribe({
        next: () => {
                    this.spinner.hide(); // ✅ HIDE SPINNER

          console.log('UPDATE SUCCESS');
          this.notificationHelper.showSuccessNotification('Blood bank updated successfully');
    ...
    this.templeservice.addnearestbloodbanks(payload)
      .subscribe({
        next: () => {
                    this.spinner.hide(); // ✅ HIDE SPINNER
...
registerbloodbanksform(): void {
  ...
  [
    ...
    whatsapp_number:['',[ Validators.pattern('^[0-9]{10}$')]],
