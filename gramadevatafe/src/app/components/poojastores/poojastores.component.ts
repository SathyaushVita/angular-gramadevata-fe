...
import { AddPoojaStoresComponent } from '../add-pooja-stores/add-pooja-stores.component';
@Component({
...
export class PoojastoresComponent {
    ...
    loadFilteredTemples() {
      ...
      if (this.selectedCategoryId && this.selectedLocationId) {
        ...
        this.templeserviceservice.poojastores(this.selectedLocationId, this.currentPage).subscribe(
          (response) => {
            this.globaltemples = response.pooja_stores;
        ...
        this.templeserviceservice.getallpoojastores().subscribe(
          (response) => {
    ...
    loadlocations(): void {
      ...
      this.locationservice.countrypooja_store('pooja_store').subscribe(
        (res) => {
  ...
  // OpenAddrestaurantDilog() {
  //   this.dialogService.open('pooja');
  // }
    // OpenAddrestaurantDilog(): void {
    //   const dialogRef = this.dialog.open(AddPoojaStoresComponent, {
    //     width: '600px', // optional
    //     data: { message: 'optional data' }, // optional
    //     autoFocus: false,
    //     backdropClass: 'dialog-backdrop' // optional custom class
 ...
 dialogRef!: MatDialogRef<any>;
    ...
    openpojastoreDialog(id: string) {
            this.currentUserId = this.authenticationService.getCurrentUser();

    this.templeserviceservice.getpoojastorebyid(id).subscribe({
      next: (res) => {
    ...
    editpooja(hotel: any): void {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result === 'updated') {
          this.openpojastoreDialog(hotel._id);
        }
      });
    }


// editpooja(tourismData: any): void {
//   if (!this.canEdit) {
//     return;
//   }

//   // ✅ Open the same pooja dialog
//   this.OpenAddrestaurantDilog();
// }
