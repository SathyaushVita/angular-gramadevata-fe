...
export class HotelsComponent {
    ...
    loadFilteredTemples() {
      ...
      if (this.selectedCategoryId && this.selectedLocationId) {
        ...
        this.templeserviceservice.hotels(this.selectedLocationId, this.currentPage).subscribe(
          (response) => {
            this.globaltemples = response.nearby_hotels;
        ...
        this.templeserviceservice.getallhotels().subscribe(
          (response) => {
    ...
    loadlocations(): void {
      ...
      this.locationservice.countryhotel('hotel').subscribe(
        (res) => {
 ...
 dialogRef!: MatDialogRef<any>;
    ...
    openhotelDialog(id: string) {
            this.currentUserId = this.authenticationService.getCurrentUser();

    this.templeserviceservice.gethotelbyid(id).subscribe({
      next: (res) => {
  ...
  edithotel(hotel: any): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'updated') {
        this.openhotelDialog(hotel._id);
      }
    });
  }
