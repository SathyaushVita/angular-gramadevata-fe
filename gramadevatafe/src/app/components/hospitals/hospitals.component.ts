...
export class HospitalsComponent {
    ...
    loadFilteredTemples() {
      ...
      if (this.selectedCategoryId && this.selectedLocationId) {
        ...
        this.templeserviceservice.Hospital(this.selectedLocationId, this.currentPage).subscribe(
          (response) => {
            this.globaltemples = response.nearby_hospitals;
        ...
        this.templeserviceservice.getallhospitals().subscribe(
          (response) => {
    ...
    loadlocations(): void {
      ...
      this.locationservice.countryhospital('hospital').subscribe(
        (res) => {
...
OpenAddhospitalDilog(): void {
 ...
 dialogRef!: MatDialogRef<any>;
    ...
    openHospitalDialog(id: string) {
            this.currentUserId = this.authenticationService.getCurrentUser();

    this.templeserviceservice.gethospitalbyid(id).subscribe({
      next: (res) => {
...
edithospital(hospital: any): void {
  ...
  dialogRef.afterClosed().subscribe(result => {
    if (result === 'updated') {
      this.openHospitalDialog(hospital._id);
    }
  });
}
