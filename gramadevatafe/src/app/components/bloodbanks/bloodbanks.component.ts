...
export class BloodbanksComponent {
    ...
    loadFilteredTemples() {
      ...
      if (this.selectedCategoryId && this.selectedLocationId) {
        ...
        this.templeserviceservice.Bloodbanks(this.selectedLocationId, this.currentPage).subscribe(
          (response) => {
            this.globaltemples = response.blood_banks;
        ...
        this.templeserviceservice.getallbloodbanks().subscribe(
          (response) => {
    ...
    loadlocations(): void {
      ...
      this.locationservice.countrypooja_store('blood_bank').subscribe(
        (res) => {
...
// OpenbloodbankDilog() {
//   this.dialogService.open('bloodbank');
// }
 ...
 dialogRef!: MatDialogRef<any>;
    ...
    openbloodbankDialog(id: string) {
      this.currentUserId = this.authenticationService.getCurrentUser();

    this.templeserviceservice.getbloodbankbyid(id).subscribe({
      next: (res) => {
    ...
    editbloodbank(hotel: any): void {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result === 'updated') {
          this.openbloodbankDialog(hotel._id);
        }
      });
    }
